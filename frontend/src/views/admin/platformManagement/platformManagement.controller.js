angular
  .module("myApp")
  .controller("carCtrl", function ($scope, ToastService, CarService, $q, $uibModal) {
   ...
   */
    ...
    /**
     * Opens rejection reason modal and handles car rejection
     * @param {Object} car - Car object to reject
     * @returns {void}
     */
    $scope.openRejectModal = (car) => {
      const modalInstance = $uibModal.open({
        templateUrl: 'views/admin/platformManagement/rejectCarModal.html',
        controller: 'rejectCarModalCtrl',
        resolve: {
          car: () => car
        }
      });

      modalInstance.result.then((rejectionData) => {
        if (rejectionData && rejectionData.reason) {
          CarService.rejectCar(car._id, rejectionData.reason)
            .then(() => {
     ...
     * @returns {void}
     */
    $scope.rejectCar = $scope.openRejectModal;
