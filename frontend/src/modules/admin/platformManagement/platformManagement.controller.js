angular.module("myApp").controller("carCtrl", function ($scope, ToastService, CarService, $q) {
  ...
  function fetchCarManagementData(){
    ...
    $q.all([  
      ...
      $scope.platformCharge.gst = charges[1].percentage;
    }).catch((err)=>{
  ...
  $scope.savePlatformCharge = (chargeName) => {
    ...
    CarService.updateCharges({charge: chargeName, percentage: chargeName === "Platform Fee" ? $scope.platformCharge.percentage : $scope.platformCharge.gst })
      .then(() => {
